...
WATCHEXEC_PID=/tmp/fql-serve-docs-watchexec.pid
BROWSERSYNC_PID=/tmp/fql-serve-docs-browsersync.pid
WATCHEXEC_LOG=/tmp/fql-serve-docs-watchexec.log
...
show_log() {
  local file
  case "${1:-}" in
    watchexec)   file=$WATCHEXEC_LOG ;;
    browsersync) file=$BROWSERSYNC_LOG ;;
    *) echo "Usage: $0 --log <watchexec|browsersync>" >&2; exit 1 ;;
  esac
  if [[ ! -f "$file" ]]; then
...
case "${1:-}" in
  ...
  "")     ;;
  *)      echo "Usage: $0 [--stop | --log <watchexec|browsersync>]" >&2; exit 1 ;;
esac

if pid_is_ours "$WATCHEXEC_PID" "$WATCHEXEC_MATCH" \
...
rm -f "$WATCHEXEC_PID" "$BROWSERSYNC_PID"
...
watchexec -w docs/index.md -w docs/index.tmpl -- ./build.sh --docs >"$WATCHEXEC_LOG" 2>&1 &
echo $! > "$WATCHEXEC_PID"
...
echo "Started."
echo "  watchexec    PID $(cat "$WATCHEXEC_PID")  log: $WATCHEXEC_LOG"
