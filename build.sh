...
If the '--down' flag is set then 'docker compose down -v' is run
first, shutting down all services and removing volumes.

If the '--image build' flag is set then the script runs 'docker
build' for the 'fql-build' docker image. The tag is determined by
the git tag/hash and the FDB version. This image is used to run
the 'generate' and 'verify' tasks below.
